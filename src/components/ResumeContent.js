...
const createSkillsSection = () => {
  ...
  const skillsList = array.map((value, index) => array[index + 1] !== undefined ? value.concat(", ") : value)
  const jsx = (
    <span className="description">{skillsList}</span>
  )
  return jsx;
}
const createHeader = () => {
  const jsx = (
    <div className="header">
      ...
      <ul className="contactDetails">
        <li><a href={resumeDataObj.site} target="_blank">{resumeDataObj.site}</a></li>
        <li><a href={resumeDataObj.email}>{resumeDataObj.email}</a></li>
        <li><a href={resumeDataObj.phone}>{resumeDataObj.phone}</a></li>
      </ul>
    </div>
  )
  return jsx;
